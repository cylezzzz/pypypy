...
class Img2ImgPipeline:
    ...
    def _resolve_model_path(self):
        ...
        if self.model_name:
            ...
            if cand_dir.exists(): return str(cand_dir)
            for p in root.rglob("*"):
                if p.is_file() and p.stem==self.model_name: return str(p.parent)
        for p in root.iterdir():
            if p.is_dir() and (p/"model_index.json").exists():
                return str(p)
        raise FileNotFoundError("Kein diffusers Image2Image Modell im models/image gefunden.")
    ...
    def ensure(self):
        ...
        try:
            self.pipe = AutoPipelineForImage2Image.from_pretrained(path, torch_dtype=dtype)
        except Exception:
            self.pipe = StableDiffusionImg2ImgPipeline.from_pretrained(path, torch_dtype=dtype)
        if dev=="cuda": self.pipe = self.pipe.to("cuda")
        self.pipe.safety_checker = None

    @torch.inference_mode()
    def run(self, init_image: Image.Image, prompt: str, strength: float=0.65, quality: str="BALANCED", negative: str|None=None, seed: int|None=None):
        self.ensure()
        ...
        result = self.pipe(
            image=init_image, prompt=prompt, negative_prompt=negative or "",
